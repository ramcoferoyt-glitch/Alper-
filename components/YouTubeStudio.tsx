...
type AssetFormat = 'post' | 'story' | 'banner' | 'thumbnail' | 'profile' | 'shorts';

const STYLES = [
    { id: 'gaming', label: 'Oyun (Gaming)', color: 'from-purple-500 to-indigo-500' },
    { id: 'vlog', label: 'Günlük Yaşam (Vlog)', color: 'from-pink-500 to-rose-500' },
...
const FORMATS: { [key in Platform]: { id: AssetFormat, label: string, ratio: AspectRatio, icon: string }[] } = {
    youtube: [
        { id: 'thumbnail', label: 'Video Kapak Resmi (Thumbnail)', ratio: '16:9', icon: 'image' },
        { id: 'shorts', label: 'Shorts (Dikey Kapak)', ratio: '9:16', icon: 'smartphone' },
        { id: 'banner', label: 'Kanal Resmi', ratio: '16:9', icon: 'panorama' },
        { id: 'profile', label: 'Profil Resmi', ratio: '1:1', icon: 'account_circle' }
    ],
    instagram: [
        { id: 'post', label: 'Kare Gönderi', ratio: '1:1', icon: 'grid_on' },
        { id: 'story', label: 'Reels / Hikaye Kapak', ratio: '9:16', icon: 'phonelink_ring' },
        { id: 'profile', label: 'Profil Resmi', ratio: '1:1', icon: 'account_circle' }
    ],
    tiktok: [
...
export const YouTubeStudio: React.FC = () => {
    ...
    const [resolution, setResolution] = useState<ImageResolution>('hd');
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            let serviceType: 'thumbnail' | 'banner' | 'profile' | 'shorts' = 'thumbnail';
            if (format === 'banner') serviceType = 'banner';
            if (format === 'profile') serviceType = 'profile';
            if (format === 'shorts') serviceType = 'shorts';
            if (format === 'story') serviceType = 'shorts'; // Map stories to vertical format
            // Post/Story treated as thumbnail/shorts logic (image gen with text) but different ratio
                ...
                <div className="p-6 space-y-8">
                    ...
                    <div>
                        <h2 className="text-xl font-bold flex items-center gap-3 mb-1 text-white">
                            <span className="material-symbols-outlined text-gray-400 text-2xl" aria-hidden="true">brush</span>
                            Sosyal Medya Tasarımcı
                        </h2>
                        <p className="text-gray-500 text-xs">YouTube, Instagram ve TikTok için kapak resimleri üret.</p>
                    </div>

                    {/* Platform Selector */}
                    <div>
                        <div className="flex bg-gray-950 p-1 rounded-xl border border-gray-800" role="radiogroup" aria-label="Platform Seçimi">
                            {PLATFORMS.map(p => (
                                <button
                                    key={p.id}
                                    role="radio"
                                ...
                                >
                                    <span className={`material-symbols-outlined text-lg ${platform === p.id ? p.color : ''}`} aria-hidden="true">{p.icon}</span>
                                    {p.label}
                                </button>
                            ))}
                        </div>
                    ...
                    {/* Format Selector */}
                    <div>
                        <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-3 block">Kapak Türü / Format</label>
                        <div className="grid grid-cols-2 gap-2" role="radiogroup" aria-label="Format Seçimi">
                    ...
                    {/* Quality Selector */}
                    <div>
                        <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-3 block">Görüntü Kalitesi</label>
                        <div className="grid grid-cols-3 gap-2">
                            <button 
                                onClick={() => setResolution('standard')}
                                className={`px-2 py-2 rounded-lg text-xs font-bold border transition-all ${resolution === 'standard' ? 'bg-blue-600 border-blue-500 text-white' : 'bg-gray-800 border-gray-700 text-gray-400'}`}
                            >
                                Standart (1K)
                            </button>
                            <button 
                                onClick={() => setResolution('hd')}
                                className={`px-2 py-2 rounded-lg text-xs font-bold border transition-all ${resolution === 'hd' ? 'bg-purple-600 border-purple-500 text-white' : 'bg-gray-800 border-gray-700 text-gray-400'}`}
                            >
                                HD (2K)
                            </button>
                            <button 
                                onClick={() => setResolution('ultra')}
                                className={`px-2 py-2 rounded-lg text-xs font-bold border transition-all ${resolution === 'ultra' ? 'bg-amber-600 border-amber-500 text-white' : 'bg-gray-800 border-gray-700 text-gray-400'}`}
                            >
                                Ultra (4K)
                            </button>
                        </div>
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2 block">Video Konusu veya Görsel Tarifi</label>
                            <textarea
                                value={visualPrompt}
                                onChange={(e) => setVisualPrompt(e.target.value)}
                                placeholder="Örn: 'Minecraft Survival İlk Bölüm', 'Evde Pizza Yapımı' veya 'Neon ışıklı sokak'..."
                                ...
                                aria-label="Görsel Tarifi veya Video Konusu"
                            ></textarea>
                        </div>
                            ...
                            >
                                {selectedImage ? (
                                    <div className="relative">
                                        <img src={selectedImage} className="h-20 mx-auto rounded object-contain" alt="Referans" />
                                        <button onClick={(e) => {e.stopPropagation(); setSelectedImage(null)}} className="absolute -top-2 -right-2 bg-red-600 rounded-full p-1" aria-label="Görseli Kaldır"><span className="material-symbols-outlined text-xs" aria-hidden="true">close</span></button>
                                    </div>
                                ) : (
                    ...
                    <button
                        ...
                        aria-label={isGenerating ? "Tasarlanıyor..." : "Oluştur"}
                    >
                        {isGenerating ? <span className="material-symbols-outlined animate-spin" aria-hidden="true">progress_activity</span> : <span className="material-symbols-outlined" aria-hidden="true">auto_awesome</span>}
            ...
            <div className="flex-grow bg-black p-6 lg:p-10 overflow-y-auto custom-scrollbar">
                <div className="flex items-center justify-between mb-6">
                     <h3 className="text-xl font-bold text-white flex items-center gap-2">
                        <span className="material-symbols-outlined text-gray-500" aria-hidden="true">collections_bookmark</span>
                        Kapak Tasarımları
                    </h3>
                </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                    {myAssets.length === 0 && (
                        <div className="col-span-full h-96 flex flex-col items-center justify-center border-2 border-dashed border-gray-800 rounded-3xl text-gray-600 bg-gray-900/20">
                             <span className="material-symbols-outlined text-6xl mb-4 opacity-30" aria-hidden="true">brush</span>
                             <p className="font-medium">Henüz bir tasarım yok.</p>
                             <p className="text-sm opacity-50">Soldan bir platform seç ve kapak tasarla.</p>
                        </div>
                    )}
                    {myAssets.map(asset => (
                        <div key={asset.id} className="group relative bg-gray-900 rounded-2xl overflow-hidden border border-gray-800 hover:border-white transition-all shadow-xl flex flex-col">
                            <div className={`w-full bg-gray-950 flex items-center justify-center overflow-hidden flex-shrink-0 ${asset.aspectRatio === '1:1' ? 'aspect-square max-w-sm mx-auto' : 'aspect-video'}`}>
                                <img src={asset.url} alt="Tasarım" className="w-full h-full object-contain" />
                            </div>
                            ...
                            <div className="p-5 flex-grow flex flex-col items-center justify-center text-center bg-gray-900/90 border-t border-gray-800">
                                ...
                                <a 
                                    ...
                                    aria-label="Tasarımı İndir"
                                >
                                    <span className="material-symbols-outlined" aria-hidden="true">download</span> İndir / Kaydet
                                </a>
                            </div>
